...
//     // find user with the given id from user table
//     let user = await this.UsersService.findUserByID(createCompanyDto.createdBy);
//     if (!user) {
//       return {
//         status: 404,
...
//       relations: ['createdBy'], // Include the user who created the company
//       order: {
...
//   // Helper method (keep this if you need it elsewhere)
...
//   // If updating the createdBy field, verify the user exists and format it correctly
//   if (updateCompanyDto.createdBy) {
//     const user = await this.UsersService.findUserByID(updateCompanyDto.createdBy);
//     if (!user) {
//       return {
//         status: 404,
//         message: 'User not found',
//       };
//     }
//     // Set the relation correctly
//     updateData.createdBy = user.user;
//   }
...
//   // Optional: RESTORE (if using soft delete)
...
//   public async findAll(filters?: FilterCompanyDto) {
...
//     // If updating createdBy, fetch user from repository
//     if (updateCompanyDto.createdBy) {
//       const user = await this.userRepository.findOne({
...
//       company: await this.findOne(companyId),
//     };
//   }
...
//   public async getEmployees(companyId: number) {
//     const company = await this.findCompanyByID(companyId);
...
//   public async getStatistics(id: number) {
//     const company = await this.companyRepository.findOne({
//       where: { id },
