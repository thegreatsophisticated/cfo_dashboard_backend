...
export class TransactionService {
  ...
  async getGlobalCashBook(
    ...
    const totalCashIn = companyReports.reduce(
      (sum, c) => sum + c.totalCashIn,
      0,
    );
    const totalCashOut = companyReports.reduce(
  ...
  private getMostActiveCompany(transactions: Transaction[]) {
    ...
    const mostActive = Object.entries(companyTransactions).sort(
      ([, a]: any, [, b]: any) => b - a,
    )[0];
    ...
    const company = transactions.find(
      (t) => t.company.id === Number(mostActive[0]),
    )?.company;
    return {
  ...
  private getLeastActiveCompany(transactions: Transaction[]) {
    ...
    const leastActive = Object.entries(companyTransactions).sort(
    ...
    const company = transactions.find(
      (t) => t.company.id === Number(leastActive[0]),
    )?.company;
    return {
  ...
  private calculateAverageTaxRate(transactions: Transaction[]) {
    const taxableTransactions = transactions.filter(
      (t) => t.taxRate && t.taxRate > 0,
    );

    if (taxableTransactions.length === 0) return '0';

    const totalTaxRate = taxableTransactions.reduce(
      (sum, t) => sum + (t.taxRate || 0),
      0,
    );
  ...
  private getTaxByCategory(transactions: Transaction[]) {
    ...
    Object.values(taxData).forEach((data: any) => {
      if (data.taxRates.length > 0) {
        data.averageTaxRate = (
          data.taxRates.reduce((sum: number, rate: number) => sum + rate, 0) /
  ...
  async getCompanyComparison(year: number, currentUser: User) {
    ...
    const topByRevenue = [...companyMetrics].sort(
      (a, b) => b.revenue - a.revenue,
    );
    const topByProfit = [...companyMetrics].sort(
      (a, b) => b.netProfit - a.netProfit,
    );
    const topByMargin = [...companyMetrics].sort(
